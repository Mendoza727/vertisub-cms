...
<div id="floating-buttons" class="floating-buttons">
    ...
    if ($wa_query->have_posts()) :
        while ($wa_query->have_posts()) : $wa_query->the_post();
            $number = get_post_meta(get_the_ID(), 'whatsapp_number', true); // número
            $message = get_post_meta(get_the_ID(), 'whatsapp_message', true); // mensaje opcional
            $wa_link = 'https://wa.me/' . $number;
            // Si quieres mensaje incluido: 
            // $wa_link = 'https://wa.me/' . $number . '?text=' . urlencode($message);
    ?>
            <a href="<?php echo esc_url($wa_link); ?>" target="_blank" class="whatsapp-button" aria-label="Contactar por WhatsApp">
